...
def rebuild_selected_table(
    ...
    with contextlib.closing(data_db.cursor()) as cur:
        cur.execute(
            "SELECT id, tag_string FROM posts WHERE NOT is_deleted AND (NOT is_pending OR ?) AND score >= ? AND INSTR(?, rating)",
            [fetch_pending, minimum_score, allowed_ratings],
        )
        with db:
...
async def main():
    ...
    data_db = sqlite3.connect(f"file:{args.data_db}?mode=ro", uri=True)
    ...
    async with aiohttp.ClientSession() as session:
        ...
        workers = [asyncio.create_task(worker()) for _ in range(args.num_workers)]
